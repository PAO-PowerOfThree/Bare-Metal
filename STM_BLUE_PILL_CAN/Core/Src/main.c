...
/* USER CODE BEGIN PV */
// static uint8_t led_state = 0; // LED state on PA5: 0 = OFF, 1 = ON
// static uint32_t last_button_press = 0; // Debounce variable
// static uint16_t adc_value = 0; // ADC reading from potentiometer on PA0
static uint8_t led_state = 0; // LED state on PA5
static uint32_t last_button_press = 0; // Debounce variable
static uint16_t adc_value = 0; // ADC reading from PA0
static uint32_t last_pwm_value = 0; // Last PWM value for change detection
static uint16_t last_adc_value = 0; // Last ADC value for change detection
  ...
  while (1)
{
    if (HAL_GPIO_ReadPin(BUTTON_GPIO_Port, BUTTON_Pin) == GPIO_PIN_RESET &&
            (HAL_GetTick() - last_button_press) > 200)
        {
            ...
            char debug[32];
            snprintf(debug, sizeof(debug), "Button: %d\r\n", led_state);
...
void send_can_message(uint32_t id, uint8_t *data, uint8_t len)
{
  ...
  // Attempt to add message to transmit mailbox
